...
**反向阀门 = 毕业（promote）。** 一条记忆满足下面任一条，就把它「毕业」：内容并进对应的 `docs/` 或 `AGENTS.md`，然后**把原记忆文件删掉或缩成一行指针**：
...
全局指令、工作空间 AGENTS.md、项目 AGENTS.md 构成一个层级规则体系。规则不是只读背景：它引用的项目会被删掉（死引用）、它定的约定会被后来的实践悄悄违反（漂移）、上下两级会互相打架（矛盾）。没人审计的规则会退化成装饰品。

处理规则层有一条铁律：**规则的真身永远在层级 AGENTS.md 里，本 skill 不复制任何具体规则内容**——复制会制造两处真相，规则一改副本就烂，这正是洁癖要消灭的头号病。所以你的做法永远是：**现场读规则 → 提取可核验项 → 核对实践 → 处置**（具体流程在第二步）。
...
**超尺寸是这个 skill 的最高优先级，大于"补本次会话漏掉的同步"。** 原因：`MEMORY.md` 超 25KB 的部分根本不进上下文（静默丢失），超尺寸的 AGENTS.md 让真正的规则被叙事段挤出 adherence——两种情况下，同步再补都徒劳。
...
- **必备文件**：如每个项目必须有 AGENTS.md、线上项目必须在顶部声明 URL → 检查存在性和内容
- **同源约束**：如 AGENTS.md 必须是指向 AGENTS.md 的软链 → `readlink` 核对
