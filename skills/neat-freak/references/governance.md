...
SKILL.md 第二步的展开。核心铁律再说一遍：**规则的真身在层级 AGENTS.md 里，本文件不复制任何具体规则内容**——这里只有「怎么提取、怎么核验、怎么处置」的方法。规则改了，这套方法不用改。
...
| 声明一致 | "AGENTS.md 里写的启动命令/URL/路径必须真实" | 对照代码与配置核验 |
...
- 补 `AGENTS.md → AGENTS.md` 软链
- 给缺 AGENTS.md 且已有可运行代码的项目建最小脚手架（工作空间规则有模板就按模板）
...
- **合并两份内容不一致的 AGENTS.md / AGENTS.md**：需要人工确认哪边是权威、哪些差异是有意的
...
### 规范审计
- 自动修复：
  - readclip 补 AGENTS.md 软链（依据：全局 AGENTS.md「同源」节）
- 待你拍板：
  - FIFA_World_Cup 目录名违反 kebab-case（依据：code/AGENTS.md 命名约定）。
