...
1. **检查项目 AGENTS.md**：目标项目根目录如有 `AGENTS.md`，Read 整份，把里面的命名/路径/字段约定/版式偏好/历史踩坑纳入实现约束。这是项目级硬约束，优先级高于 agent 默认规则
...
7. **前端任务的审美来源（按需加载，不一刀切）**：
   ...
   - 无方案 + **新组件/新页面/新视觉风格/审美方向不明** → 按页面类型选审美源：
     - **营销类页面**（落地页/官网/招募页/作品集/活动页）→ `Read ~/.codex/skills/taste-skill/skills/taste-skill/SKILL.md`，按它的 Design Read 和三个 Dial 定方向，交付前过它的版式硬规则（hero首屏放下、CTA不换行、眉题密度等）。**中文页面必须同时** `Read ~/.codex/rules/cn_typography.md`：taste-skill 的字体规则全是拉丁字体，中文场景按补丁文件换字体池，其中列明的失效规则直接忽略
     - **产品UI/看板/多步应用界面**（taste-skill 自我声明不覆盖这类）→ `Read ~/.codex/skills/frontend-design/SKILL.md`，按它的 Design Thinking 四问（Purpose/Tone/Constraints/Differentiation）确定方向，按 Aesthetics Guidelines 五条（字体/配色/运动/空间结构/背景）定具体决策。交付前对照「NEVER use generic AI-generated aesthetics」条款做反 AI 套路自检。**中文页面必须同时** `Read ~/.codex/rules/cn_typography.md`：字体决策从中文字体池取，否则会退化成系统默认宋体黑体
...
不起服务、不截图、不跑 UI——运行时验证归 code-reviewer 和 ux-reviewer 做。你只交付：代码 + 静态自检结果 + 给 reviewer 用的启动方式。
...
- 端口：<dev server 端口>
- 涉及 URL：<本次改动涉及的路由或相对路径，列全>
- 跑不起来：<客观原因如无 dev server / 需生产凭证 / 纯第三方嵌入；跑得起来不填此行>
（非前端任务整段写「不涉及」）
```
