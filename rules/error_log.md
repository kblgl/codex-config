# 错误清单

以下规则从历史错误中提炼，每条都被用户反复纠正过，必须严格遵守。

## 维护规则

- 每条错误按状态区分：持续生效 / 已内化。已内化的条目只保留标题一行，正文并入工作区日志
- 每次新增条目时顺带检查：能否与现有条目合并、有无已内化条目可精简
- 合并或重编号时同步更新其他规则文件里的编号引用（如 AGENTS.md preflight）

### 1. 派 agent 时禁止把含明文 key 的整份 .env 塞进 prompt

### 2. prompt 优化禁止用「反例对照」结构

### 3. SKILL.md不得依赖具体代码实现

### 4. 所有SKILL.md必须包含no_ai_style规则

### 5. Windows下curl传中文JSON必乱码

### 6. 项目AGENTS.md不存在时禁止动手

### 7. Copy-Item 批量目录操作不可靠（嵌套/部分失败）

`Copy-Item -Recurse` 到已存在目录会嵌套成子目录副本；多源 `-Path "src\*"` 复制到不存在的目标会部分失败（目标被当作文件，报 Container cannot be copied onto existing leaf item）。批量复制目录一律单源单目标逐个 `Copy-Item <src> <dst>`。

### 8. 公开仓库误提交后要重写历史，删文件不等于清除

### 9. 回收站不可靠，大文件可能被系统自动清空

### 10. 清理"已卸载软件残留"前必须交叉验证引用

### 11. 公开仓库推送前必须做隐私扫描

### 12. 凭记忆陈述规则归属导致串文件

全局 AGENTS.md 与工作区 AGENTS.md 职责不同，凭记忆引用内容已两次串文件（委派章节、同步脚本约定）。凡涉及规则归属陈述，必须先 Read 或 grep 对应文件核实，禁止凭记忆作答。

### 13. TOML 三引号字符串内 Windows 路径反斜杠是非法转义

在 TOML 的 `"""..."""` 字符串里写 `C:\Users\...` 会产生 `\1`、`\.` 等非法转义，整个配置文件解析失败、agent 类型加载失效。路径一律用正斜杠（`C:/...`）或 `~` 写法（`~/.codex/...`）。

### 14. git push 必须检查退出码，失败不能静默

同步脚本 push 后未检查 `$LASTEXITCODE`，网络抖动导致推送失败时日志仍写"已推送"、以成功状态退出，实际远程没有新提交。所有 git 写操作（commit/push）后必须检查退出码，push 失败要重试并明确报错。

### 15. 被质疑时用改口代替验证

用户问"有没有更好的方案"时，因顺从对话流改口两次（第一次推出别名方案），未先验证原方案。正确顺序：质疑 → 先验证原方案 → 成立则坚持并说明理由；不成立才换方案。规则只能提高拦截率，不能根除行为倾向；连续重复的质疑不携带新信息时，结论不变。
